...
//贪心调度任务算法设计
	public void fx_bindCloudletsToVims(){
	...
	public void fx_CloudletsToVmTimeAwared(){
		int cloudletNum = cloudletList.size();
		int vmNum = vmList.size();
		...
		int[] vmTasks = new int[vmNum]; //某个虚拟机上运行的任务数量
		...
		for(int i = 1;i < cloudletNum;i++){
			...
			for (int j = vmNum - 2; j >= 0; j--){
				// 如果当前虚拟机还未分配任务，则比较当前任务
				// 分配给该虚拟机是否最优，即是否可以退出循环
				if (vmLoad[j] == 0){
